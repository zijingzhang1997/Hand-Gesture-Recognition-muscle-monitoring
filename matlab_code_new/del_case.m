...
del=[];
% if you delete case 1 
if strcmp(ExpDate,'emg3') & R==2
del_temp=find(caseNum_all==7);    
del(end+1)=del_temp(34);  
del(end+1)=del_temp(35);  
fprintf(' Exp %s Routine %d\n',ExpDate,R);
end 

% if you not delete case 1 
% if strcmp(ExpDate,'emg3') & R==2
% del_temp=find(caseNum_all==7);    
% del(end+1)=del_temp(34);  
% del(end+1)=del_temp(35);  
% fprintf(' Exp %s Routine %d\n',ExpDate,R);
% end 
if strcmp(ExpDate,'2_19') & R==4
del_temp=find(caseNum_all==1);    
del(end+1)=del_temp(11);  
del(end+1)=del_temp(12); 
fprintf(' Exp %s Routine %d\n',ExpDate,R);
end 

if strcmp(ExpDate,'emg2') & R==2
del_temp=find(caseNum_all==1);    
del(end+1)=del_temp(28);  
 
fprintf(' Exp %s Routine %d\n',ExpDate,R);
end 


if strcmp(ExpDate,'emg1') & R==2
del_temp=find(caseNum_all==1);  
del(end+1)=del_temp(37);   
del_temp=find(caseNum_all==5);  
del(end+1)=del_temp(7);
del_temp=find(caseNum_all==7);  
del(end+1)=del_temp(16);del(end+1)=del_temp(17);del(end+1)=del_temp(18);
del(end+1)=del_temp(19);del(end+1)=del_temp(20);del(end+1)=del_temp(21);
del(end+1)=del_temp(22);del(end+1)=del_temp(23);del(end+1)=del_temp(24);
fprintf(' Exp %s Routine %d\n',ExpDate,R);
end 
if strcmp(ExpDate,'emg1') & R==4
del_temp=find(caseNum_all==1);    
