...
% opt.delay(:)=-1; % if you have a -1 (second) shift for all epochs 
...
toff=[4:204]';  % truncate time: start from 4s, end 5s before the end of whole routine
...
h_w(1)=plotWaveform_s(Ch_data_filt(:,ind),Chan_Name(ind),ind,fsDS,['self CH'],gesture); % self channels 
h_w(2)=plotWaveform_s(Ch_data_filt(:,ind2),Chan_Name(ind2),ind2,fsDS,['cross CH'],gesture); % cross channels channels 
ind_comp=[1,6,11,16,2,5,12,11];

h_w(2)=plotWaveform_s(abs(Ch_data_complex(:,ind_comp)),Chan_Name(ind_comp),ind_comp,fsDS,['self CH'],gesture);
[Ch_data_epoch_complex_new,Ch_data_complex_new]=VI_cor_all(Ch_data_epoch_complex,Ch_data_complex,label);
h_w(1)=plotWaveform_s(Ch_data_complex_new(:,ind_comp),Chan_Name(ind_comp),ind_comp,fsDS,['self CH'],gesture);
