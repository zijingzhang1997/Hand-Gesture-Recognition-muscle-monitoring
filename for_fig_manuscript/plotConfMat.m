...
[row,col]=find(confpercent~=0);
ind_txt=find(confpercent~=0);

hStrings = text(row,col,textStrings(ind_txt,:), ...
...
textColors=zeros(length(ind_txt),3);
for i=1:length(ind_txt)
textColors(i,:) = confmat(row(i),col(i));
end
